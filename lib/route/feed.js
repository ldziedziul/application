...
router.get('/:token/ical.ics', function(req, res){
  ...
  let leaveTypeMap = {};
  let createCacheMap = leaveTypes => {
    leaveTypes.forEach(leaveType => {
      leaveTypeMap[leaveType.id] = leaveType;
    })
  };
  Promise
    .resolve()
    .then(() => model.LeaveType.findAll().then(createCacheMap))
    ...
    .then(function(days){
      ...
      days.forEach(function(day){
        ...
        function formatLeaveTypes(day) {
          let leaveTypeIds = [day.morning_leave_type_id, day.afternoon_leave_type_id].filter(Boolean);
          let uniqueLeaveTypeIds = Array.from(new Set(leaveTypeIds));
          return uniqueLeaveTypeIds.map(id => {
            return leaveTypeMap[id].name;
          }).join(" and then ");
        }
        ...
        cal.createEvent({
          ...
          allDay  : allDay,
          summary: day.user.full_name() + ' is out of office - ' + formatLeaveTypes(day),
          status: day.is_new_leave ? 'tentative' : 'confirmed',
