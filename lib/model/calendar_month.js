          ...
          let leave_type_ids = [];
            if ( self.is_leave_morning(i) ) {
              ...
              day.leave_color_class_morning = morning_leave_type_id && self._leave_types_map[ morning_leave_type_id ]
                ...
                : 'leave_type_color_1';
              if (morning_leave_type_id && self._leave_types_map[morning_leave_type_id]) {
                leave_type_ids.push(morning_leave_type_id);
              }
            }
            ...
            if ( self.is_leave_afternoon(i) ) {
              ...
              day.leave_color_class_afternoon = afternoon_leave_type_id && self._leave_types_map[ afternoon_leave_type_id ]
                ...
                : 'leave_type_color_1';
              if (afternoon_leave_type_id && self._leave_types_map[afternoon_leave_type_id]) {
                leave_type_ids.push(afternoon_leave_type_id);
              }
            }

            if (self.is_leave(i)) {
              day.leave_obj.leave_type_name = Array.from(new Set(leave_type_ids))
                .map(id => self._leave_types_map[id] ? self._leave_types_map[id].name : '')
                .filter(Boolean)
                .join(" and then ");
            }
